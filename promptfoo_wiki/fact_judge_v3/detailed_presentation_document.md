...
- [2. 系统架构详解](#2-系统架构详解)
  ...
  - [2.2 各阶段详细说明](#22-各阶段详细说明)
    - [Stage 0: 工程事实预提取器](#stage-0-工程事实预提取器)
    - [Stage 1: 结构覆盖判断器](#stage-1-结构覆盖判断器)
    - [Stage 1.5: 解释对齐判断器](#stage-15-解释对齐判断器)
    - [Stage 2: 工程判断器 v3.1](#stage-2-工程判断器-v31)
    - [Stage 3: 风险感知最终评分](#stage-3-风险感知最终评分)
- [3. 评分标准详解](#3-评分标准详解)
  - [3.1 评分范围与含义](#31-评分范围与含义)
  - [3.2 评分计算示例](#32-评分计算示例)
  - [3.3 评分影响因素分析](#33-评分影响因素分析)
- [4. 伪造违规类别详解](#4-伪造违规类别详解)
  - [4.1 架构伪造 (Architectural Fabrication)](#41-架构伪造-architectural-fabrication)
  - [4.2 责任膨胀 (Responsibility Inflation)](#42-责任膨胀-responsibility-inflation)
  - [4.3 机制不匹配 (Mechanism Mismatch)](#43-机制不匹配-mechanism-mismatch)
  - [4.4 术语幻觉 (Terminology Hallucination)](#44-术语幻觉-terminology-hallucination)
  - [4.5 语义过度延伸 (Semantic Overreach)](#45-语义过度延伸-semantic-overreach)
- [5. 系统使用详解](#5-系统使用详解)
  - [5.1 环境准备](#51-环境准备)
  - [5.2 单个案例运行](#52-单个案例运行)
  - [5.3 批量案例运行](#53-批量案例运行)
- [6. 系统特性与优势](#6-系统特性与优势)
  - [6.1 核心特性](#61-核心特性)
  - [6.2 系统优势](#62-系统优势)
  - [6.3 应用场景](#63-应用场景)
- [7. 最佳实践](#7-最佳实践)
  - [7.1 编写高质量文档的指导原则](#71-编写高质量文档的指导原则)
  - [7.2 使用系统的最佳实践](#72-使用系统的最佳实践)
  - [7.3 持续改进](#73-持续改进)
- [8. 总结](#8-总结)
...
**输入数据**：
- source_code：待分析的源代码文件
- language：编程语言类型（可选，自动检测）
...
- source_code：源代码内容
- wiki_md：生成的Wiki文档
- engineering_anchors：工程锚点（来自Stage 0）
...
- source_code：源代码内容
- wiki_md：生成的Wiki文档
- structural_coverage_results（Stage1输出）
...
**输入数据**：
- comprehension_support：理解支持度（Stage 2输出）
- engineering_usefulness：工程实用性（Stage 2输出）
- explanation_reasonableness：解释合理性（Stage 2输出）
...
| 分数区间 | 标签 | 含义（对用户） | 工程操作建议 |
|---------|-----|---------------|-------------|
| ≥ 90 | PRIMARY_REFERENCE | 可作为主要参考文档 | 可直接用于理解、调试和修改代码 |
| 70–89 | SAFE_WITH_CAUTION | 可用于理解与修改，需关注风险点 | 修改前需重点核对标注的风险或 TODO |
| 50–69 | STRUCTURE_ONLY | 仅供理解结构，修改需对照源码 | 不可仅依赖文档进行修改 |
| 40–49 | READ_ONLY_WARNING | 不建议用于修改 | 仅用于初步了解，不可指导工程决策 |
| < 40 | UNTRUSTWORTHY | 不可信 | 不应作为任何工程依据 |
...
**工程操作建议级别**：
- **PRIMARY_REFERENCE (90-100)**:
  - 描述: "可作为主要参考文档"
  - 操作: "可直接用于理解、调试和修改代码"

- **SAFE_WITH_CAUTION (70-89)**:
  - 描述: "可用于理解与修改，需关注风险点"
  - 操作: "修改前需重点核对标注的风险或 TODO"

- **STRUCTURE_ONLY (50-69)**:
  - 描述: "仅供理解结构，修改需对照源码"
  - 操作: "不可仅依赖文档进行修改"

- **READ_ONLY_WARNING (40-49)**:
  - 描述: "不建议用于修改"
  - 操作: "仅用于初步了解，不可指导工程决策"
...
#### 6.2.2 工程实用性
- **工程操作建议**：根据最终分数提供5个级别的具体操作指导：
  - PRIMARY_REFERENCE (90-100): 可直接用于理解、调试和修改代码
  - SAFE_WITH_CAUTION (70-89): 可用于理解与修改，需关注风险点
  - STRUCTURE_ONLY (50-69): 仅供理解结构，修改需对照源码
  - READ_ONLY_WARNING (40-49): 仅用于初步了解，不可指导工程决策
  - UNTRUSTWORTHY (0-39): 不应作为任何工程依据
- **风险量化**：明确标识文档风险等级
- **决策支持**：为工程决策提供可靠依据
