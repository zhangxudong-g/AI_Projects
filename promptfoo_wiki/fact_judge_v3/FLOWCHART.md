...
8. **判断对齐** → **Stage 2**: 执行工程判断v3，使用stage2_explanatory_judge.yaml
...
graph TB
    ...
    subgraph "Stage 0: 前置提取事实"
        A3[prepare_engineering_facts.py]
        A4[提取工程级锚点和事实]
    end
...
- **Stage 2**: 使用stage2_explanatory_judge.yaml配置文件，接收Stage 1和Stage 1.5的JSON结果，通过promptfoo eval执行工程判断v3，再使用extract.py提取JSON格式结果
...
sequenceDiagram
    ...
    participant stage2 as Stage 2 (Engineering Judge v3)
...
4. run_single_case_pipeline.py脚本依次执行：
   ...
   - 接收Stage 1.5返回结果后，调用Stage 2 (Engineering Judge v3) 进行工程判断
