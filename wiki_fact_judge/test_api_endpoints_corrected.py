...
def test_execute_functionality():
    ...
    # 创建临时文件用于上传测试
    temp_dir = Path("data/test_exec_temp")
    temp_dir.mkdir(exist_ok=True)
    
    source_file = temp_dir / "exec_source.txt"
    wiki_file = temp_dir / "exec_wiki.md"
    yaml_file = temp_dir / "exec_config.yaml"
    
    source_file.write_text("# Test Source Code\nThis is a test source code file for execution.")
    wiki_file.write_text("# Test Wiki Doc\nThis is a test wiki documentation for execution.")
    yaml_file.write_text("test: true\nvalue: 123\nexecution: true")
    ...
    created_case = response.json()
    case_id = created_case['case_id']
    print(f"[PASS] 成功创建测试案例: {created_case['name']} (ID: {case_id})")
    ...
    if response.status_code == 200:
        ...
        print(f"[PASS] 成功执行测试案例，报告ID: {exec_result.get('report_id', 'N/A')}")
    else:
        print(f"[FAIL] 执行测试案例失败: {response.status_code}, {response.text}")
    ...
    created_plan = response.json()
    plan_id = created_plan['id']
    print(f"[PASS] 成功创建测试计划: {created_plan['name']} (ID: {plan_id})")
    ...
    if response.status_code == 200:
        exec_result = response.json()
        print(f"[PASS] 成功执行测试计划，报告ID: {exec_result.get('report_id', 'N/A')}")
    else:
        print(f"[FAIL] 执行测试计划失败: {response.status_code}, {response.text}")
        # 这可能是正常的，因为我们可能没有实际的数据文件
        print("  (这可能是由于缺少实际的数据文件导致的，继续测试其他功能)")
