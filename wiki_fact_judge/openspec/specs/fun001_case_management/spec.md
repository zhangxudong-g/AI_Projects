...
**行为说明**:
- 删除案例时是否同步删除文件由配置项 `DELETE_CASE_FILES_SYNC` 控制
- 默认值：`false`（仅删除数据库记录，保留文件）
- 设置为 `true` 时：同时删除数据库记录和关联文件
...
**行为说明**:
- 批量删除时是否同步删除文件由配置项 `DELETE_CASE_FILES_SYNC` 控制
- 文件删除失败时记录错误日志，但不影响数据库删除操作
...
- 删除案例时，根据配置项 `DELETE_CASE_FILES_SYNC` 决定是否同步删除文件：
  - `false`（默认）：仅删除数据库记录，保留文件
  - `true`：同时删除数据库记录和关联文件
- 文件删除失败时记录错误日志，但不影响数据库删除操作
...
1. ~~**文件清理机制**: 删除案例时是否同步删除文件？建议添加配置项控制~~ - **已解决**: 添加配置项 `DELETE_CASE_FILES_SYNC` 控制
