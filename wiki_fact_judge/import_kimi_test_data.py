...
# -*- coding: utf-8 -*-
"""
从 test_kimi 目录自动导入测试数据到 case 中
...
该脚本会:
1. 扫描 test_kimi/code 和 test_kimi/wiki 目录 (包括子目录)
2. 根据文件名匹配代码和对应的 wiki 文档
3. 自动创建 case 记录到数据库，tag 设置为 kimi
"""
...
def detect_language_from_filename(filename: str, subdir: str = '') -> str:
    ...
    Args:
        filename: 文件名 (如：Controller_xxx.java.md 或 GKBFKGZSHNT.SQL.md)
...
def import_test_data(dry_run: bool = False, default_tag: str = 'kimi'):
    """
    导入测试数据
    ...
    """
    # 目录路径
    test_dir = project_root / "test_kimi"
        ...
        for i, match in enumerate(matches, 1):
            ...
            try:
                ...
                case_data = TestCaseCreate(
                    case_id=case_id,
                    name=name,
                    tag=default_tag,  # 设置默认 tag 为 kimi
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='从 test_kimi 目录自动导入测试数据到 case 中'
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',
    ...
    parser.add_argument(
        ...
        type=str,
        default='kimi',
        help='为导入的 case 设置 tag 值 (默认：kimi)'
    )
    ...
    print(f"测试数据目录：{project_root / 'test_kimi'}")
